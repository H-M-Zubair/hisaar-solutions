      ...
      <section className="border-b border-line bg-surface">
        <div className="mx-auto max-w-[1120px] px-5 py-16 sm:px-8">
          ...
          <p className="mt-3 max-w-xl text-sm text-mute">
            Locked means upgrade, not missing forever. Shift close is on Starter so
            the drawer has a name. FBR is an add-on row, not a silent omission.
          </p>
                ...
                {matrix.map((row) => (
                  <tr key={row.feature} className="border-t border-line">
                    ...
                    <td
                      className={cn(
                        "px-4 py-3",
                        row.starter === "Full" && row.feature.startsWith("Shift close")
                          ? "text-teal"
                          : "text-mute",
                      )}
                    >
